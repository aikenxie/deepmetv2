...
def future_savez(i,dataset):
    ...
    event_list = []
    n_particles = len(events_selected.PFCands.pt[i])
    nevents = len(events_selected)
    print('Event:',i,'of ', nevents,'number of PF candidates:',n_particles)
    ...
    for j in range(n_particles):
        ...
        for jlep in range(options.n_leptons_subtract):
            dr2 = DeltaR2(events_selected.PFCands.eta[i][j], events_selected.PFCands.phi[i][j], leptons[jlep].eta, leptons[jlep].phi)
            if dr2 < 0.0001:
                islepton = True
                break
        if not islepton:
            particle_list = [
                events_selected.PFCands.pt[i][j],
                events_selected.PFCands.eta[i][j],
                events_selected.PFCands.phi[i][j],
                events_selected.PFCands.mass[i][j],
                events_selected.PFCands.d0[i][j],
                events_selected.PFCands.dz[i][j],
                events_selected.PFCands.pdgId[i][j],
                events_selected.PFCands.charge[i][j],
                events_selected.PFCands.pvAssocQuality[i][j],
                events_selected.PFCands.puppiWeight[i][j]
            ]
            event_list.append(particle_list)
        #else:
        #    print ("jlep: ", jlep, " dr2 ", dr2)

    npz_file = '/hildafs/projects/phy230010p/xiea/npzs/'+dataset+'/raw/'+dataset+'_nevent_'+str(i)+'_nParticles_'+str(len(events_selected))
    print('Saving file', npz_file+'.npz')
    return np.savez(npz_file, x = np.array(event_list), y = np.array(genmet_list))

'''
select events with numbers of leptons >= nlepcut
'''
def SelectEvent(nlepcut):
    ...
    select_tight_muon = ((events.Muon.tightId == 1) & (
        events.Muon.pfRelIso03_all < 0.15) & (events.Muon.pt > 20.)) #mask for selecting tight muons

    muonCopy = events.Muon[:]
    muonCopy['istight'] = select_tight_muon #muons per event that passes the mask

    events['Muon']=muonCopy[muonCopy.istight]
    
    # select electrons
    select_tight_electron = (
        (events.Electron.mvaFall17V1Iso_WP80 == 1) & (events.Electron.pt > 20.0))
    
    electronCopy = events.Electron[:]
    electronCopy['istight'] = select_tight_electron #electrons per event that passes the mask

    events['Electron'] = electronCopy[electronCopy.istight]

    num_leptons = ak.count(events.Electron.pt,axis=-1) + ak.count(events.Muon.pt,axis=-1)
        
    events_selected = events[num_leptons >= nlepcut]
...
if __name__ == '__main__':
    ...
    parser.add_option('--nn',help ='number of nano aod file, e.g 10', dest='nn',default=0 )
    (options, args) = parser.parse_args()

    dataset=options.dataset
    nanoNumber=options.nn

    datasetsname = {'dy':'/hildafs/projects/phy230010p/share/DYJetsToLL/','tt':'/hildafs/projects/phy230010p/share/TTTo2L2Nu/'}
    ...
    fname = datasetsname[dataset]+'output_nano_'+str(nanoNumber)+'.root'
    ...
    n_events = ak.num(events_selected.Muon.pt,axis=0)

    print('Total events selected:', n_events)

    
    for i in range(n_events):
    #for i in range(10):
        future_savez(i,dataset)
    '''
